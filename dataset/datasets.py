...
def load_data(data_path=cfg.data_path, X_recon_path = cfg.X_recon_path, data_index=None, data_name=None):
    '''
    Args:
    ...
    '''
    X = sio.loadmat(
        data_path+X_recon_path+'/{}/{}_{}.mat'.format(data_name, data_name, data_index))[data_name]  # [Kx,Ky,T]
...
def train_dataloader(data_index_list, data_path, X_recon_path, data_name, batch_size_default=cfg.batch_size):
    '''
    由加载的数据构造训练数据集
    Args:
    ...
    for data_index in data_index_list:
        X, para_maps, max_X = load_data(data_path, X_recon_path, data_index, data_name)
        yield (X, para_maps, max_X)


def test_dataloader(data_index_list, data_path, X_recon_path, data_name, batch_size_default=cfg.batch_size):
    '''
    由加载的数据构造测试数据集
    Args:
    data_index_list: 数据集索引列表
